...
1. 在右側「控制台」點擊「● 開始錄製」
2. 在目標視窗正常操作（點擊、輸入文字、按快捷鍵等）；ClickFlow 視窗內的動作會自動排除
3. 按 `F9` 或點擊「■ 停止錄製」結束錄製
4. 錄製完成後步驟自動填入序列，可直接儲存或手動微調

**錄製選項**

| 選項 | 說明 |
|------|------|
| 錄製移動 | 勾選後額外記錄游標移動軌跡（預設關閉） |
| 延遲上限 | 超過此秒數的停頓統一壓縮為上限值（預設 5 秒） |

> 連續輸入的文字會自動合併為一個 `keyboard_input` 步驟；雙擊會自動識別，不會拆成兩次單擊。
